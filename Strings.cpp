...
// '.' Matches any single character.（匹配任意的单个字符）
// '*' Matches zero or more of the preceding element.（匹配*号前面字符任意多次）
bool Strings::isMatch(string s, string p)
{
	return isMatch(s.c_str(), p.c_str());
}
bool Strings::isMatch(const char *s, const char *p)
{
	if (*p == '\0') return *s == '\0';
